...
* Система уже работает хорошо, но требуются специалисты (3-5 человек), которые разбираются в специфике рынка труда в представляющих интерес областях (например, общепит, документооборот, строительство). Они должны помочь расширить граф понятий, более адекватно его структурировать.

* Для создания крупной системы (если окажется, что небольшой системы будет недостаточно) потребуется создать маленький язык программирования и некоторые средства работы с ним. Здесь нужны 1-2 человека, имевших опыт с редакторами кода и создании интерпретаторов/компиляторов. 
