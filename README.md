...
На практике используется очень простой алгоритм:

1. Для каждой n-граммы проводится **стемминг**, то есть выделение основы, с учётом того, что n-грамма может быть как словом, так и группой слов.

1. Все результаты стемминга объединяются *во множество*. Таким образом, получается некий **мешок информации** о вакансии/резюме.

1. Остаётся узнать, есть ли в этом мешке навыки (или маркеры), которые представляют интерес для нашей задачи. То есть является ли *основа* (результат стемминга) какого-либо узла графа подмножеством этого мешка.

1. Если произошло совпадение с каким-то навыком, этот навык выводится. Если произошло совпадение с каким-то маркером, этот маркер всё равно приведёт к одному или нескольким навыкам (потому что так устроен граф), которые выведутся.
...
Идея здесь в том, чтобы для незнакомых основному алгоритму понятий (предположительно IT-терминов) искать совпадения с темами из Википедии, используя разные вариации расстояния Левенштейна. Если некоторые условия выполнены, будет выведен результат.

Этот процесс хорошо кешируется, поэтому периодически можно смотреть, есть ли какие-то популярные темы, не охваченные основным алгоритмом (если тема популярна и не охватывается основным алгоритмом, ответ будет искать вспомогательный), и добавлять их в граф. Таким образом, **в процессе работы системы будет происходить её обучение**. Это особенно удобно, потому что сам *основной алгоритм устроен так, что с обновлением данных становится только лучше*. Кроме того, *кеширование гарантирует, что уже увиденные запросы не будут требовать сетевых ресурсов при повторении, поэтому вся система будет работать быстро*.

# Почему не нейронная сеть?

**Для нейронных сетей задача выделения из текста навыков определённой тематики** (в данном случае, профессиональных навыков) **пока едва ли разрешима**. По сути, это задача многоклассовой классификации со множественным выходом, где количество классов может достигать нескольких сотен (насколько я знаю, сейчас есть сети, поддерживающие только десяток классов). Отсюда исходят следующие причины против использования нейронной сети:

* Для как минимум приемлемых результатов потребуется огромный объём размеченных данных, так что требуется, чтобы группа людей просмотрела не меньше 10 000 резюме и каждому резюме поставила в соотвествие набор навыков, причём сперва требуется договориться о том, какие именно навыки нужны и какими именно словами их выражать.

* Даже если такой датасет будет создан, а нейронная сеть правильно составлена и обучена (что совсем непросто сделать), всё, к чему эта нейронная сеть придёт, это выявление зависимости между такими-то словами и таким понятием и между таким понятием и таким поняем. То есть то, что должна будет обнаружить нейронная сеть, это то, что мы и так можем записать в наш граф, потому что мы это уже знаем благодаря нашей биологии и нашему опыту. Я говорю о том, что **глупо создавать нейронную сеть, если мы можем сразу записать результаты на основе наших знаний, что будет быстрее, дешевле и даст большее качество**. 
