...
# Как этим пользоваться
  ...
  Основная функция модуля — ```get_content_from_text``` из файла [detector.py](https://github.com/PasaOpasen/ContentDetector/blob/master/content_detector/detector.py). Главной её задачей является вывод результата, представимого в виде списка навыков(каждый навык — это просто строка)  из входящих в неё списков содержащих резюме.
...
-  Набор строк увеличивается за счёт **разбиения каждой строки по запятым (',')**. Важно чтобы конструкции, перечисляемые через запятую, не содержали искомую информации в совокупности — только отдельную друг от друга.

-  Аналогично происходит **разбиение по стоп-словам** (если в строке есть стоп-слово, оно удаляется, слова которые он разделяет, становятся отдельными строками). Используемые стоп-слова хранятся [в файле](https://github.com/PasaOpasen/ContentDetector/blob/master/content_detector/stopwords(used).txt); Важно заметить, что сначала использовались только стоп-слова русского и английского языков из **NLTK**, но для получения более приемлемого результата, были добавлены основные русские предлоги и русские символы. Этот список можно при необходимости расширять и редактировать.
